...
export const api = {
    ...
    startThirtySeconds: (playerNames: string[], roundsPerPlayer: number) =>
        request('/thirty-seconds/games/start', {
            method: 'POST',
            body: JSON.stringify({ mode: 'SOLO', playerNames, roundsPerPlayer })
        }),
    ...
    completeGame: (gameId: string) =>
        request(`/thirty-seconds/games/${gameId}/complete`, { method: 'POST' }),
    startQna: (questionCount: number, durationSeconds: number) =>
        request('/qna/attempts/start', {
            method: 'POST',
            body: JSON.stringify({ questionCount, durationSeconds })
        }),
    submitQna: (attemptId: string, answers: Array<{ questionId: string; selectedOptionId: string; timeTakenMs: number }>) =>
